...
public class UndrawIllustrationTool {
    ...
    public List<ImageResource> searchIllustrations(@P("搜索关键词") String query) {
        ...
        try (HttpResponse response = HttpRequest.get(apiUrl).timeout(10000).execute()) {
            ...
            log.error("搜索插画失败，返回默认插画");
            // 兜底策略
            return getDefaultIllustration();    // mmp的，为什么浏览器测试可以，编码的时候就不行啊
        }
        return imageList;
    }
    ...
    private List<ImageResource> getDefaultIllustration() {
        log.info("使用默认插画：https://undraw.co/illustrations/happy_day.svg");
        return Arrays.asList(
                ImageResource.builder()
                        .category(ImageCategoryEnum.ILLUSTRATION)
                        .description("默认插画：Happy Day")
                        .url("https://undraw.co/illustrations/happy_day.svg")
                        .build()
        );
    }
}
